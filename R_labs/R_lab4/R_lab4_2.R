...
if (file.exists("данные.xlsx")) {
  # Загрузка данных
  data <- read_excel("данные.xlsx", sheet = "Стьюдент4")
  ...
  print(t.test(x, y, alternative = "two.sided", var.equal = TRUE))
  
  cat("Односторонняя альтернатива (µx < µy):\n")
  print(t.test(x, y, alternative = "less", var.equal = TRUE))
  
  cat("Односторонняя альтернатива (µx > µy):\n")
  print(t.test(x, y, alternative = "greater", var.equal = TRUE))
